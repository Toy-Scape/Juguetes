...
namespace Inventory
{
    ...
    public class InventoryExample : MonoBehaviour
    {
        ...
        #region Ejemplos Avanzados

        /// <summary>
        /// EJEMPLO AVANZADO: Sistema de crafting básico.
        /// </summary>
        /// <remarks>
        /// Demuestra cómo usar el inventario para implementar un sistema de crafting
        /// que verifica materiales y crea items nuevos.
        /// </remarks>
        [Header("Ejemplo de Crafting")]
        [SerializeField] private ItemData material1;
        [SerializeField] private int material1Required = 2;
        [SerializeField] private ItemData material2;
        [SerializeField] private int material2Required = 1;
        [SerializeField] private ItemData craftedItem;

        [ContextMenu("Intentar Craftear Item")]
        public void TryCraftItem()
        {
            // Verificar si tenemos todos los materiales necesarios
            int material1Count = playerInventory.GetItemCount(material1);
            int material2Count = playerInventory.GetItemCount(material2);

            if (material1Count >= material1Required && material2Count >= material2Required)
            {
                // Tenemos suficientes materiales, proceder con el crafting
                Debug.Log("✓ Materiales suficientes. Crafteando item...");

                // Eliminar los materiales
                playerInventory.RemoveItem(material1, material1Required);
                playerInventory.RemoveItem(material2, material2Required);

                // Añadir el item crafteado
                bool success = playerInventory.AddItem(craftedItem, 1);

                if (success)
                {
                    Debug.Log($"✓ Has crafteado: {craftedItem.ItemName}");
                }
                else
                {
                    // Si no se pudo añadir (inventario lleno), devolver los materiales
                    Debug.LogWarning("✗ Inventario lleno. Devolviendo materiales...");
                    playerInventory.AddItem(material1, material1Required);
                    playerInventory.AddItem(material2, material2Required);
                }
            }
            else
            {
                Debug.LogWarning($"✗ Materiales insuficientes. Necesitas: {material1Required}x {material1?.ItemName ?? "null"} (tienes {material1Count}) y {material2Required}x {material2?.ItemName ?? "null"} (tienes {material2Count})");
            }
        }

        /// <summary>
        /// EJEMPLO AVANZADO: Verificación de múltiples items (Quest).
        /// </summary>
        /// <remarks>
        /// Demuestra cómo verificar si el jugador tiene varios items específicos,
        /// útil para sistemas de quests o misiones.
        /// </remarks>
        [Header("Ejemplo de Quest")]
        [SerializeField] private ItemData[] questItems;

        [ContextMenu("Verificar Quest Completada")]
        public void CheckQuestCompletion()
        {
            if (questItems == null || questItems.Length == 0)
            {
                Debug.LogWarning("No hay items de quest asignados.");
                return;
            }

            bool questComplete = true;
            Debug.Log("=== VERIFICANDO QUEST ===");

            foreach (var questItem in questItems)
            {
                if (questItem == null) continue;

                bool hasItem = playerInventory.Contains(questItem);
                Debug.Log($"{(hasItem ? "✓" : "✗")} {questItem.ItemName}: {(hasItem ? "Obtenido" : "Falta")}");
            ...
            if (questComplete)
            {
                Debug.Log("✓✓✓ ¡QUEST COMPLETADA! Todos los items han sido recolectados.");
                // Aquí puedes dar la recompensa, avanzar la historia, etc.
            }
            else
            {
                Debug.Log("Quest incompleta. Continúa buscando los items faltantes.");
            }
        }
