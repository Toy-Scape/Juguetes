    ...
    public class RowPropPlacer : BuildingUnit
    {
        ...
        void DrawPreviewMeshGizmo(GameObject reference)
        {
            if (reference != lastPreviewReference)
            {
                ...
                generatedPreviewGo.transform.localScale = BHUtilities.BMTFindUniformScale(transform.localScale);
                tracker.owner = this;
            }
            if (generatedPreviewGo == null)
                return;
            ...
            {
                if (generatedPreviewRenederers[i] == null || generatedPreviewRenederers[i].sharedMesh == null)
                    continue;
