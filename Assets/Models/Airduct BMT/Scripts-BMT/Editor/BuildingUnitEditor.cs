...
namespace BuildingMakerToolset.PropPlacer {
    ...
    public class BuildingUnitEditor : UnityEditor.Editor
    {
        ...
        {
            Undo.undoRedoPerformed += OnPeroformUndoRedo;
        }
        ...
        void OnPeroformUndoRedo()
        {
            BuildingUnit tgt = target as BuildingUnit;
            tgt.ForceUpdate();
        }
        ...
        private void OnSceneGUI()
        {
            ...
            BuildingUnit bu = target as BuildingUnit;
            ScenePickingMode(bu);
            CheckForHierarchyConflictInSceneGUI(eventType, bu);
        }


        static GUIStyle _sceneInfoPupup;
        static GUIStyle SceneInfoPupup
        {
            get
            {
                if (_sceneInfoPupup == null)
                {
                    _sceneInfoPupup = new GUIStyle("HelpBox");
                    _sceneInfoPupup.alignment = TextAnchor.MiddleCenter;
                    _sceneInfoPupup.fixedWidth = 320;
                }
                return _sceneInfoPupup;
            }
        }
        static bool displayCantMoveWarning = false;
        public static void CheckForHierarchyConflictInSceneGUI(EventType eventType, BuildingUnit buildingUnit)
        {

            if (displayCantMoveWarning)
                Handles.Label(buildingUnit.transform.position + Vector3.up * 0.35f, "This objects inherits its position/rotation from an other object. To move independtly, disable TransformAwareness in the inspector.", SceneInfoPupup);
            if (eventType == EventType.MouseDown)
            {
                displayCantMoveWarning = false;
            }
            if (eventType == EventType.MouseDrag && Selection.activeGameObject == buildingUnit.gameObject)
            {
                displayCantMoveWarning = false;
                if (buildingUnit.IsRelatedInHierarchy())
                {
                    displayCantMoveWarning = true;
                }
            }
        }
